...
let
  ...
  background = "${config.home.homeDirectory}/.dotfiles/wallpapers/flowerbed.jpg";

  lock = "${pkgs.swaylock-effects}/bin/swaylock --debug --clock --image ${background} --indicator --indicator-thickness 7 --indicator-radius 150 --effect-vignette 0.4:0.4 --font 'monospace' --datestr '%a, %Y-%m-%d'";

  screenshot_destination = "${config.home.homeDirectory}/Images/screenshots/scrn-$(date +\"%Y-%m-%d-%H-%M-%S\").png";

  gtk_theme_name = "Mint-Y-Dark-Red";
