...
DEVICE_TREE_NAME = "@deviceTreeName@"
...
# This gathers the store dir no matter how deep the file is
def store_dir_from_path(path: str) -> str:
    result = path
    while True:
        p = os.path.dirname(result)
        if p == '/nix/store':
            return os.path.basename(result)
        if p == '/':
            break
        result = p
    raise ValueError(f"path not in /nix/store: {path}")

def copy_from_file(file: str, dry_run: bool = False) -> str:
    store_file_path = os.path.realpath(file)
    suffix = os.path.basename(store_file_path)
    store_dir = store_dir_from_path(store_file_path)
...
def write_entry(profile: str | None, generation: int, specialisation: str | None,
    ...
    devicetree = None
    if DEVICE_TREE_NAME != "":
        devicetree = copy_from_file(os.path.join(
            bootspec.toplevel, 'dtbs',DEVICE_TREE_NAME,
        ))
...
def remove_old_entries(gens: list[SystemIdentifier]) -> None:
    ...
    if DEVICE_TREE_NAME != "":
        known_paths.append(copy_from_file(os.path.join(
            bootspec.toplevel, 'dtbs',DEVICE_TREE_NAME,
        ),True))
...
def install_bootloader(args: argparse.Namespace) -> None:
    try:
        with open("/etc/machine-id") as machine_file:
            machine_id = machine_file.readlines()[0].rstrip()
    except IOError as e:
